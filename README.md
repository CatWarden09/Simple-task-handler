...
Запускает все программы под .exe, указанным в коде, и расположенные по указанному пользователем пути в подпапках с цифровым индексом (см.[ **Настройка папок**](#совместимость)). Запуск производится одновременно, поэтому команда не рекомендуется к использованию на слабых ПК.
